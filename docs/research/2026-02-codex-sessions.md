...
**Status**: Implemented — Codex session parsing and token extraction shipped in v0.0.12 (pending). Known gap: sessions are global, not scoped per-project. See also [2026-02-container-and-session-discovery.md](2026-02-container-and-session-discovery.md) for container/remote environment considerations.
